...
async def run():
    ...
    async with async_playwright() as p:
        ...
        await say("左京区北白川、公園西の駐車場。11頭くらい、耳カットは2匹、子猫はいない。3人でやります")
        await say("餌場はうちの玄関脇1か所（自分の家の敷地）。18時に2人で、30分後に容器を回収して残りは持ち帰る")
        await shot("k1_chat")
        await say("庭にトイレ2つ、朝7時に掃除。苦情は私に")
        await say("北白川町内会の代表の高橋修さん（左京区北白川◯◯町10-1、075-000-0000）に、私が8/23に説明して同意あり。回覧板で年1回周知")
        await say("代表は私、山田花子。左京区北白川◯◯町1-2、090-0000-0000。佐藤一郎さん、左京区北白川◯◯町3-4、090-1111-1111、捕獲と搬送担当。鈴木良子さん、北区上賀茂◯◯町5-6、090-2222-2222、餌やり交代。全員京都市民で、鈴木さんだけ町内の住民ではない")
        ...
        await say("10頭手術した。新しく1匹来た。死亡・譲渡なし。苦情なし")
        if await pg.locator("button", has_text="へ提出").count() == 0:
            await say("はい、その内容で間違いありません")
        ...
        await say("高島市今津町、琵琶湖岸の民宿裏の空き地。6頭、手術済みなし、子猫3匹")
        await say("餌場は民宿の裏口1か所、民宿の主人の承諾あり。朝6時と夕方5時、30分後に回収。トイレは空き地の隅に1つ、朝6時に掃除。病院は隣の市、車で40分")
        await say("個人です。森綾子、高島市今津町◯◯在住、090-3333-3333。町内会への説明はまだ")
