...
const fs = require('fs');
const IMG = '/home/user/VORN/docs/deck2/';
const P = new pptxgen();
...
const sizes={ 'next.jpg':[1400,588],'rules.jpg':[1400,803],'form1.jpg':[1215,1120],'chat.jpg':[910,1210],'chat_s.jpg':[900,1197],'docs.jpg':[1400,635],'tk_sim.jpg':[1400,373],'tk_rules.jpg':[1400,560],'city_list.jpg':[1400,448],'city_detail.jpg':[1400,719],'city_pending.jpg':[1400,317],'city_reported.jpg':[1400,317] };
...
{ const s=slide(true);
  ...
  img(s,'next.jpg',8.6,1.5,4.1); cap(s,'市民面：開くと「次にやること」が出る',8.6,1.5+imgH('next.jpg',4.1)+0.05,4.1,0.3);
  foot(s,'',1);
}
// 2 何を作ったか
{ const s=slide();
  title(s,'会話で状況を入れると、次にやることと書類が出て、市の台帳に残る。',{h:0.7});
  const y=1.45, w=3.75, g=0.31;
  frame(s,'①','担い手が話す','chat_s.jpg',ML,y,w,4.4);
  cap(s,'「左京区、11頭くらい、3人でやります」と話す。AIが要綱に照らして返す',ML,y+4.75,w,0.5);
  frame(s,'②','AIが要綱を読んで組み立てる','next.jpg',ML+w+g,y,w,1.58); cover(s,'form1.jpg',ML+w+g,y+2.0,w,2.7);
  cap(s,'「次にやること」と、埋まった第1号様式',ML+w+g,y+4.75,w,0.5);
  frame(s,'③','市に残る','city_list.jpg',ML+2*(w+g),y,w,1.2); cover(s,'city_pending.jpg',ML+2*(w+g),y+1.62,w,0.85);
  cap(s,'K-2026-010 が「申請中」で台帳に載り、報告後に「報告済」になる',ML+2*(w+g),y+2.55,w,0.6);
  foot(s,'プロトタイプ（Streamlit）。京都市・高島市の2自治体で動作。画面はすべて実機。',2);
}
// 2 構造
{ const s=slide();
...
{ const s=slide();
  ...
  foot(s,'出典：京都市「まちねこ活動支援事業の10年間の事業評価」／堺市 猫の困りごとページ／福岡県調査',4);
}

// 5 摩擦①
{ const s=slide();
  title(s,'少ない担い手が制度の時間軸に合わせて動かないと、猫の方が先に増える。',{h:0.7});
  bullets(s,['基金チケットは申請が毎月1〜5日、翌月1か月のみ有効。自治体はさらに独自の期日（高島市は翌々月分）。','窓を1回逃すと手術は1か月遅れ、次の出産が先に来る。'],ML,1.5,5.6,2.2,13);
  card(s,ML,3.9,5.6,1.2); text(s,'「いつ妊娠するか分からないので受付期間を限定しないでほしい」\n福岡県調査・40代女性',ML+0.2,4.0,5.2,1.0,{size:12});
  const fx=7.2, fy=2.4, fw=5.4;
  ...
  foot(s,'出典：どうぶつ基金 チケットFAQ・規約／高島市 環境政策課ページ／福岡県調査',5);
}
// 6 摩擦②③④
{ const s=slide();
  title(s,'制度は届いておらず、続かず、担い手が消えれば情報ごと消える。',{h:0.7});
  bullets(s,['自治体差：出る市町村と出ない市町村、要綱も様式も別。協力病院は全国200件弱。「病院が遠すぎて連れていけない」','継続：報告・更新・完了報告を落とせば制度から外れる。京都市要綱では更新がなければ職権で抹消（第7条3項）。','引継ぎ：担い手が倒れると、どの猫が手術済みか・餌場はどこかの情報ごと消える。'],ML,1.5,6.8,3.8,13);
  card(s,8.0,1.5,4.6,1.7); text(s,'10.0% ＞ 8.6%',8.2,1.6,4.2,0.8,{size:28,bold:true,valign:'middle'}); text(s,'活動しているのに制度を知らない（35件） ＞ 知っている（30件）。福岡県調査',8.2,2.45,4.2,0.6,{size:10,color:MUTED});
  card(s,8.0,3.4,4.6,1.7); text(s,'3年横ばい',8.2,3.5,4.2,0.8,{size:28,bold:true,valign:'middle'}); text(s,'京都市の活動中地域数。毎年新規はあるのに増えない。10年評価',8.2,4.35,4.2,0.6,{size:10,color:MUTED});
  foot(s,'出典：福岡県調査／京都市10年評価／京都市まちねこ活動支援要綱 第7条',6);
}
// 5 原因
{ const s=slide();
  ...
  foot(s,'出典：京都市まちねこ活動支援事業ページ／京都市まちねこ活動支援要綱（令和6年4月1日改正）',7);
}
// 6 解決策
{ const s=slide(true);
...
// 9 画面：会話
{ const s=slide();
  title(s,'担い手は、要綱を読まない。場所と頭数を話すだけ。',{h:0.7});
  ...
  text(s,'入力は自然文。要綱の条件は AI 側が持つ。集めるのは第1号様式・第6号様式に必要な項目だけで、それ以外は聞かない。\n\n捕獲の手順・自治会の説得・里親探し・子猫の育て方を聞かれると「範囲外」と断る。',6.6,3.3,6.0,2.0,{size:12,color:MUTED});
  foot(s,'画面はプロトタイプ。デモ用の架空コロニー（左京区・11頭・活動者3名）',9);
}
// 10 画面：判定
{ const s=slide();
  title(s,'不足と注意が、条文の根拠付きで返る。',{h:0.7});
  cover(s,'rules.jpg',ML,1.4,CW,5.4);
  callout(s,'「周辺地図は別途添付」は要綱第5条、「登録後に第4号様式」は第9条から。判定は要綱PDFから毎回抽出し、13件の規則それぞれに出典と引用が付く。規則はコードに書いていない。',8.6,5.2,3.9,1.4);
  foot(s,'画面はプロトタイプ（京都市プロファイル）',10);
}
// 11 画面：次にやること
{ const s=slide();
  title(s,'開くと「次にやること」が1つだけ出る。',{h:0.7});
  img(s,'next.jpg',ML,1.4,CW);
  callout(s,'期限は登録日から要綱の期日ルールで自動計算。報告は同月日から30日以内、更新は満了30日前から。',8.6,1.6,3.8,1.0);
  cap(s,'何を・いつまでに・様式は何。台帳の状態が変わると項目が入れ替わる。提出前は「提出」、登録後は「周知」「手術申請」「報告」「更新」。',ML,1.4+imgH('next.jpg',CW)+0.1,CW,0.5);
  foot(s,'画面はプロトタイプ。登録直後（K-2026-010）',11);
}
// 12 画面：様式
{ const s=slide();
  title(s,'京都市配布のDOCXに、そのまま転記される。',{h:0.7});
  img(s,'form1.jpg',ML,1.4,5.7);
  callout(s,'様式のセル構造（表・行・列）をAIが読み、どのセルに何を書くかを決めて転記する。第6号（実施計画書）・第4号（手術実施申請書）も同様。',6.8,1.45,5.8,0.95);
  img(s,'docs.jpg',6.8,2.6,5.8); cap(s,'画面上の項目一覧。「要確認」は記載例と照らして確認が要る欄。担い手は印刷して出すだけ。',6.8,2.6+imgH('docs.jpg',5.8)+0.08,5.8,0.5);
...
{ const s=slide();
  ...
  foot(s,'出典：どうぶつ基金 2022年度実績（個人3,546名／47団体／298行政／約62,128頭）',13);
}

// 14 画面：高島
{ const s=slide();
  title(s,'同じ画面、同じコード。要綱を差し替えると、日程が変わる。',{h:0.7});
  img(s,'tk_sim.jpg',ML,1.4,CW);
  callout(s,'+30日。この受付（毎月6日〜末日）を逃すと、チケットの有効月が1か月遅れる。',ML+0.2,1.4+imgH('tk_sim.jpg',CW)-1.0,4.6,0.8);
  cover(s,'tk_rules.jpg',ML,1.4+imgH('tk_sim.jpg',CW)+0.15,CW,1.7);
  cap(s,'高島市：準備完了日を入れると、受付期間（毎月6日〜末日、翌々月分）から最短の申請日とチケット有効期間が出る。受付の値は市HP本文から抽出。高島市の交付要綱PDFは未取得。',ML,6.4,CW,0.5);
  foot(s,'画面はプロトタイプ（高島市プロファイル）。出典：高島市 環境政策課ページ／どうぶつ基金 チケットFAQ・登録規約',14);
}
// 15 自治体面 一覧
{ const s=slide();
  title(s,'市は、更新が切れそうな地域を、切れる前に知る。',{h:0.7});
  img(s,'city_list.jpg',ML,1.4,CW);
  callout(s,'更新待ち → 期限超過。届出なしで切れる地域が、切れる前に見える。',8.9,2.5,3.5,0.8);
  cap(s,'状態は要綱の期日ルールと報告履歴から計算。並び順は 期限超過 → 報告待ち → 更新待ち → 申請中 → 報告済。台帳の他4件はダミー。',ML,1.4+imgH('city_list.jpg',CW)+0.1,CW,0.5);
  foot(s,'画面はプロトタイプ（自治体面：京都市医療衛生センター）',15);
}
// 16 自治体面 詳細
{ const s=slide();
  title(s,'提出済みの様式と報告が、そのまま市に残る。',{h:0.7});
  img(s,'city_detail.jpg',ML,1.4,7.6);
  card(s,8.7,1.4,3.9,3.0); text(s,'引継ぎのために別途入力させるものではない。申請と報告の副産物として残る。\n\n活動地域数「3年横ばい」の中身（終了か脱落か）が初めて分かる。廃止届（第3号様式）には理由欄があるが、届出なしで切れた地域の理由は今は残らない。',8.9,1.55,3.5,2.7,{size:11.5});
  foot(s,'出典：京都市まちねこ活動支援要綱 第7条3項（期限までに更新されない登録の職権抹消）',16);
}
// 17 通しデモ
{ const s=slide();
  title(s,'架空コロニー K-2026-010 が、入力から報告済になるまで。',{h:0.7});
  const w=3.75, g=0.31, y=1.4;
  frame(s,'1','会話入力','chat_s.jpg',ML,y,w,2.2); frame(s,'2','判定','rules.jpg',ML+w+g,y,w,2.2); frame(s,'3','様式出力','form1.jpg',ML+2*(w+g),y,w,2.2);
  const y2=4.15, w2=5.8;
  frame(s,'4','市の一覧に「申請中」で載る','city_pending.jpg',ML,y2,w2,1.32); frame(s,'5','年次報告を出すと「報告済」に変わる','city_reported.jpg',ML+w2+0.23,y2,w2,1.32);
  cap(s,'画面はすべて実機。市の職員が「登録する」を押すと登録日が入り、次にやることに「周知」「第4号様式」「報告期限」「満了日」が並ぶ。台帳の他4件はダミー。',ML,6.0,CW,0.5);
  foot(s,'プロトタイプ（Streamlit、モック応答で撮影）',17);
}
// 10 結果と指標
{ const s=slide();
...
{ const s=slide();
  ...
  foot(s,'出典：京都市獣医師会 まちねこページ／京都市10年評価／どうぶつ基金 2022年度実績',19);
}

// 20 展開
{ const s=slide();
  title(s,'自治体プロファイルを差し替えるだけで、要綱で動く制度すべてに同じエンジンが使える。',{h:0.7});
  bullets(s,['読んでいるのは「猫の要綱」ではなく「自治体が自然言語で公開している要綱と様式」。','同じ形の制度は他にもある：住民が申請し、期限があり、報告と更新があり、担い手が無償で属人的なもの。','猫はその中で、効果が実証済みで、担い手が最も薄く、市が障壁を認めた場所。だからここから。'],ML,1.5,6.8,3.8,13);
  const fx=8.2, fy=1.8;
  box(s,fx+1.0,fy,2.2,0.8,'同じエンジン',null,{fill:NAVY,line:NAVY,color:WHITE});
  box(s,fx,fy+1.7,1.4,0.6,'京都市',null,{}); box(s,fx+1.4,fy+1.7,1.4,0.6,'高島市',null,{}); box(s,fx+2.8,fy+1.7,1.2,0.6,'……',null,{fill:WHITE,dash:'dash',color:MUTED});
  [[fx+1.5,fy+0.8,fx+0.7,fy+1.7],[fx+2.1,fy+0.8,fx+2.1,fy+1.7],[fx+2.7,fy+0.8,fx+3.4,fy+1.7]].forEach(a=>s.addShape(P.ShapeType.line,{x:Math.min(a[0],a[2]),y:a[1],w:Math.abs(a[2]-a[0]),h:a[3]-a[1],line:{color:MUTED,width:1.5},flipH:a[2]<a[0]}));
  text(s,'列挙しない。要綱一式のフォルダを置くだけ',fx,fy+2.5,4.2,0.3,{size:10,color:MUTED,align:'center'});
  foot(s,'',20);
}
// 13 チーム
{ const s=slide(true);
  ...
  text(s,'技術構成　Python／Streamlit／Anthropic API（Claude）／python-docx。京都市・高島市の2プロファイルで動作。要綱の規則はコードに書かず、資料フォルダから毎回抽出。日付の計算だけコード側。\n\n限界　当事者ヒアリングは未実施（通過後に京都市医療衛生センター・どうぶつ基金へ）。高島市は市HP本文から抽出しており、交付要綱PDFは未取得。実 API での応答品質は検証中で、掲載画面はモック応答で撮影。',ML,3.5,CW,1.6,{size:11,color:'AAB4BE'});
  foot(s,'',21);
}
// 付録A
{ const s=slide();
...
P.writeFile({fileName:'/home/user/VORN/docs/deck2/VORN2026_submission_v2.pptx'}).then(f=>console.log('written',f));
