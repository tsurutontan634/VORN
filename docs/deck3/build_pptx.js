...
{ const s=slide();
  ...
  text(s,'1自治体あたりの担い手は数十〜数百人規模にとどまる一方、協働自治体は298にのぼり、要綱はすべて異なる。汎用ツールが存在しないため、現場では電話、LINE、Excel、紙によるアナログな手作業が続いている。',ML,1.6,CW,0.6,{size:13});
...
{ const s=slide();
  ...
  frame(s,'③','自治体管理台帳への自動連携','city_list.jpg',ML+2*(w+g),y,w,1.2); cover(s,'city_pending.jpg',ML+2*(w+g),y+1.62,w,0.85); cap(s,'K-2026-010 が「申請中」で載り、報告後に「報告済」になる',ML+2*(w+g),y+2.55,w,0.6);
...
{ const s=slide();
  ...
  callout(s,'町内会の同意を伝えると記録し、続けて「頭数が11頭（10頭以上）なので、同一世帯でない地域住民2名を含む京都市民3名以上が必要」と要綱第2条⑵の条件を返してから、活動者の入力へ進む。',6.6,1.9,6.0,1.1);
