...
{ const s=slide();
  ...
  foot(s,'プロトタイプ（Streamlit／DeepSeek API）。京都市・高島市で動作。画面はプロトタイプの実画面。会話・判定・転記は deepseek-chat の実応答。',9);
}
// 10 話す
{ const s=slide();
  title(s,'要綱の読み込みは不要、活動場所と頭数を伝えるだけで手続きが進行',{h:0.7});
  cover(s,'chat.jpg',ML,1.45,5.4,2.4); img(s,'chat2.jpg',ML,4.0,5.4);
  callout(s,'3往復目で、活動者3名の氏名・住所・役割・電話と「地域住民か・京都市民か」を求めてくる。頭数11頭に対する第2条⑵の人数要件（地域住民2名を含む京都市民3名以上）は、次頁の照合で判定する。',6.6,1.9,6.0,1.1);
  ...
  foot(s,'画面はプロトタイプ。デモ用の架空コロニー（左京区・11頭・活動者3名）。会話は deepseek-chat の実応答。',10);
}
// 11 判定＋次にやること
{ const s=slide();
  title(s,'提出前に不足要件を根拠条文付きで可視化、画面を開くだけで次のタスクを提示',{h:0.7});
  img(s,'rules.jpg',ML,1.45,5.9); cap(s,'判定（不足0件・注意4件）。周辺地図の添付は要綱第5条、未手術9頭の計画は申請書裏面⑷から。R2は「11頭で10頭以上のため京都市民3人以上」を第2条⑵から判定してOK。規則ごとに出典と引用が付く。',ML,5.9,5.9,0.5);
  ...
  foot(s,'画面はプロトタイプ（京都市プロファイル）。規則抽出33件と判定は deepseek-chat の実応答。規則は要綱から毎回抽出し、コードに規則を持たない。',11);
}
// 12 様式
{ const s=slide();
  ...
  foot(s,'左：第1号様式 まちねこ活動登録申請書（配布 touroku.docx）に deepseek-chat の応答（14セル）で転記した DOCX を HTML で再現表示（Word 画面ではない）。町内会代表者の住所・電話・実施者は聞き取りで未入力のため空欄。',12);
}
// 13 市の職員
{ const s=slide();
...
{ const s=slide();
  ...
  cover(s,'tk_rules.jpg',ML,4.2,6.4,1.6); cap(s,'高島市の資料から抽出した規則に照らした判定。聞き取りを3往復で止めた状態のため、申請者区分などが未回答で不足7件。受付の値は市HP本文から抽出。交付要綱PDFは未取得。',ML,5.85,6.4,0.5);
  ...
  foot(s,'画面はプロトタイプ（高島市プロファイル）。規則抽出27件・判定・受付期間の抽出は deepseek-chat の実応答。出典：高島市 環境政策課ページ／どうぶつ基金 チケットFAQ・登録規約',15);
}
// 16 主題
{ const s=slide();
...
{ const s=slide();
  ...
  [['展開経路','京都市から立ち上げ。動物病院（京都市獣医師会）や行政の相談窓口を通じて案内。費用対効果として、自治体窓口の問い合わせ対応や不備対応に関わる事務コストを削減。'],['指標','申請から手術までの日数／登録継続率（職権抹消の減少）／引継ぎ後に継続した地域数／センター収容の子猫数'],['今後の検証課題と開発状況','審査通過後にセンター・基金への当事者ヒアリングを実施予定／高島市要綱PDFの取得と検証／LLM応答の安定化（JSONモードで空応答が返る場合の再試行、聞き取りの取りこぼし）を継続中']].forEach((c,i)=>{ const x=ML+i*4.0; card(s,x,1.5,3.8,2.5); text(s,c[0],x+0.2,1.65,3.4,0.4,{size:15,bold:true}); text(s,c[1],x+0.2,2.15,3.4,1.75,{size:12}); });
  card(s,ML,4.3,CW,1.5); text(s,'技術構成',ML+0.2,4.42,CW-0.4,0.4,{size:15,bold:true}); text(s,'Python／Streamlit／python-docx。LLM は OpenAI 互換 API（掲載画面は DeepSeek deepseek-chat で実行）と Anthropic API を切替可。京都市・高島市の2プロファイルで動作。要綱の規則はコードに書かず、資料フォルダから毎回抽出。日付の計算だけコード側。',ML+0.2,4.9,CW-0.4,0.8,{size:12});
